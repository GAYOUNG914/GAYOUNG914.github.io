$(function () {
  ...
  $("#navigation .menubox > li > a").on("click", function () {
    if ($(this).hasClass("on")) {
      slideUp();
      $(".menubox_li").removeClass("on");
    } else {
      slideUp();
      $(this).addClass("on").next().slideDown();
      // $(this).parent().addClass("on");
      //on이없는 a 가 클릭되면,
      //this a의 부모는 addclass on이 되어야하고
      //다른 a의 부모는 removeclass on이 되어야한다.
      $(this).parent().addClass("on");
      $(".menubox > li > a").not($(this)).parent().removeClass("on");
    }
